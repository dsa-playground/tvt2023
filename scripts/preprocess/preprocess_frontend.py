...
from scripts.preprocess.preprocess_clean import (
    rename_cols,
    change_order_cols,
    add_sum_column,
    drop_cols,
...
from scripts.preprocess.preprocess_collect import (
    add_multiple_records,
    transform_multiplechoice_anwser,
...
def opschonen_dataframe(df, config=_config):
    ...
    df_clean = rename_cols(
        df=df_clean, dict_renamed_columns=config["preprocess"]["clean"]["rename"]
    )
    for item in _config["preprocess"]["clean"]["add_sum_columns"]["items"].keys():
        df_clean = add_sum_column(
            df=df_clean,
            variable_name=_config["preprocess"]["clean"]["add_sum_columns"]["items"][
                item
            ]["colname"],
            list_columns_to_sum=_config["preprocess"]["clean"]["add_sum_columns"][
                "items"
            ][item]["columns"],
        )
    df_clean = df_clean.pipe(
        change_order_cols, list_order_cols=config["preprocess"]["clean"]["order"]
...
def numeriek_maken_dataframe(df, config=_config):
    ...
    df_num = (
        df_num.pipe(
            drop_cols, list_drop_cols=config["preprocess"]["clean"]["drop_cols"]
        )
        .pipe(
            fillna_cols,
            dict_fillna_strategy=config["preprocess"]["clean"]["fillna_strategy"],
        )
        .pipe(dropna_rows)
        .pipe(
            floor_values_cols, list_rounding_cols=config["preprocess"]["clean"]["floor"]
        )
        .pipe(
            replace_values_cols,
            dict_replacing=config["preprocess"]["clean"]["label_encode"],
        )
...
def vereenvoudigen_data(df_train, df_test, config=_config):
...
def aanpassen_data(df_train_clean, df_test_clean, config=_config):
...
def voeg_passagiers_toe(df_train, df_test, config=_config):
    ...
    list_workshop_passagiers_updated = transform_multiplechoice_anwser(
        ...
        dict_with_multiplechoice_anwsers=_config["preprocess"]["collect"][
            "transform_multi"
        ],
    )
    df_workshop_passagiers = transform_multi_records_to_df(
        list_with_all_new_records=list_workshop_passagiers_updated,
        list_with_drop_columns=_config["preprocess"]["collect"]["drop_cols"],
    )
    for item in _config["preprocess"]["clean"]["add_sum_columns"]["items"].keys():
        df_workshop_passagiers = add_sum_column(
            df=df_workshop_passagiers,
            variable_name=_config["preprocess"]["clean"]["add_sum_columns"]["items"][
                item
            ]["colname"],
            list_columns_to_sum=_config["preprocess"]["clean"]["add_sum_columns"][
    ...
    df_workshop_passagiers = df_workshop_passagiers.pipe(
        replace_values_cols,
        dict_replacing=config["preprocess"]["clean"]["label_encode"],
    ).pipe(create_index, list_index_cols=config["preprocess"]["clean"]["index_cols"])
